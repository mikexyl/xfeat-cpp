...
LightStereoDepth::InferenceEngine::InferenceEngine(const std::string& engine_file, bool verbose)
    : runtime_(nullptr), engine_(nullptr), context_(nullptr), 
      graph_(nullptr), instance_(nullptr), stream_(nullptr), verbose_(verbose) {
    
    // Set CUDA device
    cudaError_t status = cudaSetDevice(0);
    if (status != cudaSuccess) {
        throw std::runtime_error(std::string("Error setting CUDA device: ") + 
...
void LightStereoDepth::InferenceEngine::preprocess(const std::map<std::string, cv::Mat>& sample) {
    for (const auto& [key, image] : sample) {
        ...
        if (buffer && !image.empty()) {
            cudaMemcpyAsync(buffer, image.data, 
...
std::map<std::string, cv::Mat> LightStereoDepth::InferenceEngine::postprocess() {
    ...
    cudaMemcpyAsync(disp_pred.data, buffers_[2], 
    ...
    disp_pred.convertTo(normalized_disp_pred, CV_8UC1, 
                       255.0 / (maxVal - minVal), 
...
std::map<std::string, cv::Mat> LightStereoDepth::InferenceEngine::run(
    const std::map<std::string, cv::Mat>& sample) {
...
LightStereoDepth::TransformPipeline::TransformPipeline(
...
cv::Mat LightStereoDepth::TransformPipeline::padImage(const cv::Mat& image) {
    ...
    cv::Mat padded;
    cv::copyMakeBorder(image, padded, pad_h, 0, 0, pad_w, 
...
std::map<std::string, cv::Mat> LightStereoDepth::TransformPipeline::operator()(
...
LightStereoDepth::LightStereoDepth(const Params& params) : params_(params) {
    ...
    transform_ = std::make_unique<TransformPipeline>(
        params_.target_size, params_.mean, params_.std);
}

LightStereoDepth::~LightStereoDepth() = default;

void LightStereoDepth::compute(const cv::Mat& left, const cv::Mat& right, 
                               cv::Mat& disparity) {
    ...
    // Store original size for potential post-processing
    original_size_ = left.size();

    // Preprocess
    auto sample = (*transform_)(left, right);
    ...
    // Postprocess - crop to original size if needed
    if (raw_disparity_.size() != original_size_) {
        cv::Rect roi(0, raw_disparity_.rows - original_size_.height,
                    original_size_.width, original_size_.height);
        disparity = raw_disparity_(roi).clone();
    } else {
