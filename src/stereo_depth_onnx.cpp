...
void OnnxStereoDepth::preprocessImage(const cv::Mat& img, std::vector<float>& output) {
  ...
  for (int c = 0; c < 3; c++) {
    std::memcpy(output.data() + c * image_size, 
                channels[c].data, 
...
void OnnxStereoDepth::runInference(const std::vector<float>& left_tensor,
  ...
  auto left_tensor_obj = Ort::Value::CreateTensor<float>(
      memory_info_, 
      const_cast<float*>(left_tensor.data()), 
      left_tensor.size(),
      input_shape.data(), 
      input_shape.size());
  
  auto right_tensor_obj = Ort::Value::CreateTensor<float>(
      memory_info_,
...
void OnnxStereoDepth::postprocessDisparity(const std::vector<float>& model_output,
                                           cv::Mat& disparity) {
  ...
  std::memcpy(raw_disparity_.data, model_output.data(), 
...
void OnnxStereoDepth::computeDepthDirect(const cv::Mat& left, const cv::Mat& right, 
                                         cv::Mat& depth) {
...
void OnnxStereoDepth::warmup(const cv::Size& image_size) {
  if (params_.verbose) {
    std::cout << "Warming up ONNX stereo depth with " << params_.warmup_iterations 
              << " iterations..." << std::endl;
  }
...
cv::Mat OnnxStereoDepth::getColorDisparity() const {
  ...
  if (max_val > min_val) {
    raw_disparity_.convertTo(normalized, CV_8U, 
