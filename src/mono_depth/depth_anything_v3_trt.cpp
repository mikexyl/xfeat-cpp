...
class DepthAnythingV3TRT::Impl {
 public:
  ...
  bool hasPoseInputs() const { return hasExtrinsicsInput(); }
  ...
  std::vector<MonoDepthResult> inferViews(const std::vector<cv::Mat>& views,
    ...
    const double pose_scale =
        (!predicted_extrinsics.empty() && !world_to_camera_extrinsics.empty())
            ? mono_depth_detail::estimateInputToPredictedPoseScale(predicted_extrinsics, world_to_camera_extrinsics)
            : 1.0;
    ...
    for (size_t i = 0; i < views.size(); ++i) {
      ...
      if (!predicted_extrinsics.empty()) {
        result.predicted_world_to_camera = predicted_extrinsics[i];
      }
...
bool DepthAnythingV3TRT::has_pose_inputs() const { return impl_->hasPoseInputs(); }
