...
class gms_matcher {
 public:
  ...
  gms_matcher(const vector<KeyPoint>& vkp1,
              const Size size1,
 ...
 private:
  ...
  inline vector<int> GetNB9(const int idx, const Size& GridSize) {
    ...
    for (int yi = -1; yi <= 1; yi++) {
      for (int xi = -1; xi <= 1; xi++) {
        ...
        if (idx_xx < 0 || idx_xx >= GridSize.width || idx_yy < 0 || idx_yy >= GridSize.height) continue;
  ...
  inline void InitalizeNiehbors(Mat& neighbor, const Size& GridSize) {
...
inline void gms_matcher::VerifyCellPairs(int RotationType) {
  ...
  for (int i = 0; i < mGridNumberLeft; i++) {
    ...
    if (score < thresh) mCellPairs[i] = -2;
  }
}
